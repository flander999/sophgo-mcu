...
int top_misc_read(int idx, uint64_t offset, uint32_t *value)
{
	...
	if (err) {
		debug("failed on top misc reg read at addr 0x%lx\n", addr);
		return err;
	}
...
int top_misc_write(int idx, uint64_t offset, uint32_t value)
{
	...
	if (err) {
		debug("failed on top misc reg write at addr 0x%lx\n", addr);
		return err;
	}
		...
		*postdiv2 = 1;
		...
		if (index < ARRAY_SIZE(postdiv1_2)) {
			...
			*postdiv2 = postdiv1_2[index][0];
			...
			debug("%s out of postdiv array range!\n", __func__);
			ret = -1;
		}
	}
...
static inline int sg2044_pll_switch_mux(int idx, struct sg2044_pll_clock *pll, char en)
{
	...
	if (shift == -1) {
		debug("%s Unable to find a suitable shift!\n", __func__);
		return -1;
	}
...
int sg2044_clk_pll_set_rate(int idx, int mpll_id, uint64_t rate, uint64_t parent_rate)
{
	...
	if (sg2044_pll_enable(idx, sg2044_pll, 0)) {
		debug("Can't disable pll(%s), status error\n", sg2044_pll->name);
		ret = -1;
		goto out;
	}
	...
	if (!pctrl_table.freq) {
		debug("%s: Can't find a proper pll setting\n", sg2044_pll->name);
		ret = -1;
		goto out;
	}
